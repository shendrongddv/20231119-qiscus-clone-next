            ...
            {item.isGroup ? (
              <Popover>
                ...
                <PopoverContent
                  asChild
                  className={cn(
                    "hidden w-full max-w-2xl grid-cols-2 gap-2 rounded-lg p-2 md:grid",
                  )}
                >
                  <ul className="">
                    {item.links?.map((item) => (
    ...
    <Link
      ...
      className={cn(
        buttonVariants({
          ...
          size: "default",
          className:
            "flex h-full w-full items-start justify-start gap-2 whitespace-normal border-none",
        }),
      )}
    >
      <Icon className="h-5 w-5 shrink-0 text-primary group-hover:text-white" />
